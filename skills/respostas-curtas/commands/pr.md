...
1. Rode `git fetch`, depois o diff `<base>...HEAD` — primeiro os nomes de
   arquivo, o diff em si onde os nomes não bastarem. Leia o suficiente dos
   arquivos mudados para descrever **comportamento**, não linhas. Se a branch
   não tem commits sobre a base, diga isso e pare; nunca invente descrição.
2. Primeira linha: o que a PR faz. Depois dela, só o que o revisor não tira
